...
use common\models\MinutaLocalidadesSearch;
 ...
 */
class MinutaLocalidadesController extends Controller
{
    /**
     * @inheritdoc
     */
    ...
    {
        $searchModel = new MinutaLocalidadesSearch();
    ...
    {
        if (($model = MinutaLocalidades::findOne($id)) !== null) {
            return $model;
        }
    ...
    public function loadImage($field, $type, $model, $tipo) {
        $imageFile= UploadedFile::getInstanceByName('MinutaLocalidades['.$type.']');
        ...
        if ($imageFile) {
            ...
            if ($ext == 'pdf'){
                ...
                FileHelper::createDirectory(Yii::getAlias('@images').'/minutasl/'.$model->id);
                //var_dump($name); die;
                $pdf->saveAs(Yii::getAlias('@images').'/minutasl/'.$model->id.'/'.$name, false);
            }else{
                die;
            }
